    ...
    return(
        <div className="flex flex-col items-center h-full w-full">
            ...
            <div className="flex justify-center h-max w-full mt-10 mb-5">
                <div className="flex h-[150px] w-[150px] rounded-full border-2 transition-all overflow-hidden" onMouseEnter={()=>{setEditIcon(true)}} onMouseLeave={()=>{setEditIcon(false)}}><img className="shadow-inner hover:shadow-md transition-shadow duration-300" src={profileImage?profileImage:BigUserIcon} />{editIcon?<label htmlFor="fileInput"><img className="absolute z-10 font-bold text-2xl left-[225px] top-[200px] cursor-pointer" src={EditIcon} /></label>:null}<input type="file" id="fileInput" hidden onChange={handleFileChange}/></div>
            </div>
